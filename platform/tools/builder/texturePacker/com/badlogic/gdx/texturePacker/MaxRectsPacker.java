...
public class MaxRectsPacker {
  ...
  TexturePackerManager.Settings settings;
  private Sort sort = new Sort();
  public MaxRectsPacker (TexturePackerManager.Settings settings) {
    this.settings = settings;
    if (settings.MIN_WIDTH > settings.MAX_WIDTH) throw new RuntimeException("Page min width cannot be higher than max width.");
    if (settings.MIN_HEIGHT > settings.MAX_HEIGHT)
      throw new RuntimeException("Page min height cannot be higher than max height.");
  }
  public Array<TexturePackerManager.Page> pack (Array<TexturePackerManager.Rect> inputRects) {
    for (int i = 0, nn = inputRects.size; i < nn; i++) {
      TexturePackerManager.Rect rect = inputRects.get(i);
      rect.width += settings.paddingX;
      rect.height += settings.paddingY;
    }
    if (settings.fast) {
      if (settings.rotation) {
        sort.sort(inputRects, new Comparator<TexturePackerManager.Rect>() {
          public int compare (TexturePackerManager.Rect o1, TexturePackerManager.Rect o2) {
    ...
    Array<TexturePackerManager.Page> pages = new Array();
    while (inputRects.size > 0) {
      TexturePackerManager.Page result = packPage(inputRects);
      pages.add(result);
      inputRects = result.remainingRects;
    }
    return pages;
  }
  private TexturePackerManager.Page packPage (Array<TexturePackerManager.Rect> inputRects) {
    ...
    for (int i = 0, nn = inputRects.size; i < nn; i++) {
      ...
      if (settings.rotation) {
        if ((rect.width > settings.MAX_WIDTH || rect.height > settings.MAX_HEIGHT)
          ...
          throw new RuntimeException("Image does not fit with max page size " + settings.MAX_WIDTH + "x" + settings.MAX_HEIGHT
              + " and padding " + settings.paddingX + "," + settings.paddingY + ": " + rect);
        }
      } else {
        if (rect.width > settings.MAX_WIDTH) {
          throw new RuntimeException("Image does not fit with max page width " + settings.MAX_WIDTH + " and paddingX "
              + settings.paddingX + ": " + rect);
        }
        if (rect.height > settings.MAX_HEIGHT && (!settings.rotation || rect.width > settings.MAX_HEIGHT)) {
          throw new RuntimeException("Image does not fit in max page height " + settings.MAX_HEIGHT + " and paddingY "
    ...
    TexturePackerManager.Page bestResult = null;
    while (true) {
      TexturePackerManager.Page bestWidthResult = null;
      while (width != -1) {
        TexturePackerManager.Page result = packAtSize(true, width - edgePaddingX, height - edgePaddingY, inputRects);
  ...
  private TexturePackerManager.Page packAtSize (boolean fully, int width, int height, Array<TexturePackerManager.Rect> inputRects) {
    TexturePackerManager.Page bestResult = null;
    for (int i = 0, n = methods.length; i < n; i++) {
      maxRects.init(width, height);
      TexturePackerManager.Page result;
      if (!settings.fast) {
        result = maxRects.pack(inputRects, methods[i]);
      } else {
        Array<TexturePackerManager.Rect> remaining = new Array();
        for (int ii = 0, nn = inputRects.size; ii < nn; ii++) {
          TexturePackerManager.Rect rect = inputRects.get(ii);
  ...
  private TexturePackerManager.Page getBest (TexturePackerManager.Page result1, TexturePackerManager.Page result2) {
  ...
  class MaxRects {
    ...
    private final Array<TexturePackerManager.Rect> usedRectangles = new Array();
    private final Array<TexturePackerManager.Rect> freeRectangles = new Array();
    public void init (int width, int height) {
      binWidth = width;
      binHeight = height;
      usedRectangles.clear();
      freeRectangles.clear();
      TexturePackerManager.Rect n = new TexturePackerManager.Rect();
      n.x = 0;
      n.y = 0;
      n.width = width;
      n.height = height;
    ...
    public TexturePackerManager.Rect insert (TexturePackerManager.Rect rect, FreeRectChoiceHeuristic method) {
      TexturePackerManager.Rect newNode = scoreRect(rect, method);
      ...
      pruneFreeList();
      TexturePackerManager.Rect bestNode = new TexturePackerManager.Rect();
      bestNode.set(rect);
    ...
    public TexturePackerManager.Page pack (Array<TexturePackerManager.Rect> rects, FreeRectChoiceHeuristic method) {
      ...
      while (rects.size > 0) {
        ...
        for (int i = 0; i < rects.size; i++) {
          TexturePackerManager.Rect newNode = scoreRect(rects.get(i), method);
      ...
      TexturePackerManager.Page result = getResult();
      result.remainingRects = rects;
      return result;
    }
    public TexturePackerManager.Page getResult () {
      int w = 0, h = 0;
      for (int i = 0; i < usedRectangles.size; i++) {
        TexturePackerManager.Rect rect = usedRectangles.get(i);
        w = Math.max(w, rect.x + rect.width);
        h = Math.max(h, rect.y + rect.height);
      }
      TexturePackerManager.Page result = new TexturePackerManager.Page();
    ...
    private void placeRect (TexturePackerManager.Rect node) {
      int numRectanglesToProcess = freeRectangles.size;
      for (int i = 0; i < numRectanglesToProcess; i++) {
    ...
    private TexturePackerManager.Rect scoreRect (TexturePackerManager.Rect rect, FreeRectChoiceHeuristic method) {
      ...
      TexturePackerManager.Rect newNode = null;
      switch (method) {
    ...
    private TexturePackerManager.Rect findPositionForNewNodeBottomLeft (int width, int height, int rotatedWidth, int rotatedHeight, boolean rotate) {
      TexturePackerManager.Rect bestNode = new TexturePackerManager.Rect();
      bestNode.score1 = Integer.MAX_VALUE;
    ...
    private TexturePackerManager.Rect findPositionForNewNodeBestShortSideFit (int width, int height, int rotatedWidth, int rotatedHeight,
                                                                        boolean rotate) {
      TexturePackerManager.Rect bestNode = new TexturePackerManager.Rect();
      bestNode.score1 = Integer.MAX_VALUE;
      for (int i = 0; i < freeRectangles.size; i++) {
    ...
    private TexturePackerManager.Rect findPositionForNewNodeBestLongSideFit (int width, int height, int rotatedWidth, int rotatedHeight,
                                                                       boolean rotate) {
      TexturePackerManager.Rect bestNode = new TexturePackerManager.Rect();
      bestNode.score2 = Integer.MAX_VALUE;
      for (int i = 0; i < freeRectangles.size; i++) {
    ...
    private TexturePackerManager.Rect findPositionForNewNodeBestAreaFit (int width, int height, int rotatedWidth, int rotatedHeight, boolean rotate) {
      TexturePackerManager.Rect bestNode = new TexturePackerManager.Rect();
      bestNode.score1 = Integer.MAX_VALUE;
      for (int i = 0; i < freeRectangles.size; i++) {
    ...
    private TexturePackerManager.Rect findPositionForNewNodeContactPoint (int width, int height, int rotatedWidth, int rotatedHeight, boolean rotate) {
      TexturePackerManager.Rect bestNode = new TexturePackerManager.Rect();
      bestNode.score1 = -1;
      for (int i = 0; i < freeRectangles.size; i++) {
        if (freeRectangles.get(i).width >= width && freeRectangles.get(i).height >= height) {
    ...
    private boolean splitFreeNode (TexturePackerManager.Rect freeNode, TexturePackerManager.Rect usedNode) {
      if (usedNode.x >= freeNode.x + freeNode.width || usedNode.x + usedNode.width <= freeNode.x
          || usedNode.y >= freeNode.y + freeNode.height || usedNode.y + usedNode.height <= freeNode.y) return false;
      if (usedNode.x < freeNode.x + freeNode.width && usedNode.x + usedNode.width > freeNode.x) {
        if (usedNode.y > freeNode.y && usedNode.y < freeNode.y + freeNode.height) {
          TexturePackerManager.Rect newNode = new TexturePackerManager.Rect(freeNode);
    ...
    private boolean isContainedIn (TexturePackerManager.Rect a, TexturePackerManager.Rect b) {
  ...
  class RectComparator implements Comparator<TexturePackerManager.Rect> {
    @Override
    public int compare (TexturePackerManager.Rect o1, TexturePackerManager.Rect o2) {
